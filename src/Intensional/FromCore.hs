...
consInstArgs x as k = mapM fromCoreInst (dataConRepArgTys k)
  where
    ...
    fromCoreInst (Tcr.TyConApp d as')
        | isTypeSynonymTyCon d, Just (as'', s) <- synTyConDefn_maybe d = fromCoreInst
            (substTy (extendTvSubstList emptySubst (zip as'' as')) s)
        | -- Instantiate type synonym arguments
          isClassTyCon d = return Ambiguous
        | -- Disregard type class evidence
          otherwise = do
            b <- isIneligible d
            if b
                then Data (Base d) <$> mapM fromCoreInst as'
...
fromCore f (Tcr.TyConApp d as)
    | isTypeSynonymTyCon d, Just (as', s) <- synTyConDefn_maybe d = fromCore
        f
        (substTy (extendTvSubstList emptySubst (zip as' as)) s)
...
fromCore Nothing (Tcr.TyConApp d as) = do
    ...
    if b
        ...
        else Data (Inj x d) <$> mapM (fromCore Nothing) as
fromCore (Just x) (Tcr.TyConApp d as) = do
    b <- isIneligible d
    if b
        then Data (Base d) <$> mapM (fromCore (Just x)) as
        else Data (Inj x d) <$> mapM (fromCore (Just x)) as
